...
`Demuxer`, `Muxer`) plus the registry. Each codec/container crate
exposes `pub fn register(&mut RuntimeContext)` and the
[`oxideav-meta`](https://github.com/OxideAV/oxideav-meta) aggregator
calls every enabled sibling's `register` from a build-script-generated
`register_all(ctx)`. The pipeline executor only sees the registry,
never any concrete crate.
...
Or pull every codec/container/filter/source via the
[`oxideav-meta`](https://github.com/OxideAV/oxideav-meta) aggregator:

```toml
[dependencies]
oxideav-core = "0.1"
oxideav-meta = "0.0"             # default-features = ["all"] pulls everything
```
...
Slimmer subsets via Cargo features. Preset bundles (`audio`, `video`,
`image`, `subtitles`, `hwaccel`, `source-drivers`) plus per-crate flags:

```toml
# Just image codecs.
oxideav-meta = { version = "0.0", default-features = false, features = ["image"] }

# MP4 + H.264 + AAC only.
oxideav-meta = { version = "0.0", default-features = false, features = ["mp4", "h264", "aac"] }
```

`register_all` is auto-generated by `oxideav-meta`'s `build.rs` from
its `Cargo.toml` + active feature flags — only the siblings you opted
into get linked.
...
| [`oxideav-meta`](https://github.com/OxideAV/oxideav-meta) | Aggregator: `register_all(ctx)` over every enabled codec / container / filter / source. Cargo presets (`audio` / `video` / `image` / `subtitles` / `hwaccel` / `all`) + per-crate flags. |
| [`oxideav-videotoolbox`](https://github.com/OxideAV/oxideav-videotoolbox) | macOS hardware video codec bridge (VideoToolbox; runtime-loaded via libloading, priority 0 — preferred over pure-Rust on Mac). |
| [`oxideav-audiotoolbox`](https://github.com/OxideAV/oxideav-audiotoolbox) | macOS hardware audio codec bridge (AudioToolbox; same shape as the video bridge). |
